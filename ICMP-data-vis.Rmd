...
total.table <- ICMP.dump %>% # this one gets a total count
...
types.table <- ICMP.dump %>% # this one gets a types count
  filter(TypeStatus != "") %>%
  group_by(SpecimenType) %>%
  dplyr::summarize(
    Types = n(), 
...
NZ.table <- ICMP.dump %>% # this one gets a New Zealand count
...
NZtype.table <- ICMP.dump %>% # NZ and types
