...
types.table <- ICMP.dump %>% # this one get a types count
  filter(TypeStatus != "") %>%
  group_by(SpecimenType) %>%
  dplyr::summarize(
    Types = n(), 
...
NZ.table <- ICMP.dump %>% # this one get a total count
...
NZtype.table <- ICMP.dump %>% # NZ types
...
kable(icmp.count.table, digits=0, "html",
  ...
  add_header_above(c(" " = 2,  "number of cultures" = 3))
