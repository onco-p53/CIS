...
This is an R Markdown document for statistics derived from specimen data of the ICMP culture collection <https://www.landcareresearch.co.nz/icmp>. This is an experimental showcase of presenting visualisations of data analysed in R in a user-friendly format.
...
The ICMP has **`r count(ICMP.types)` type cultures** cultures, these consist of:

```{r types table, echo=TRUE}
...
Many ICMP cultures are associated with a host plant, for example rhizobia in legume nodules, or plant pathogenic fungi causing leaf spots. Below are the top 10 most common host plant families in the ICMP with type of organism.

```{r top 10 host families, echo=FALSE}
...
Each of the cultures from New Zealand that have a geocoordinate are plotted below. A better interative map can be found on GBIF at: <https://www.gbif.org/dataset/3c6e7390-3b56-11dc-8c19-b8a03c50a862>.

```{r nz map, echo=FALSE, warning=FALSE}
...
```{r top NZ area codes, echo=FALSE, warning=FALSE}
...
```{r calulate dates, echo=FALSE}

date.isolated <- ymd(ICMP.dump$IsolationDateISO, truncated = 3)
sort(table(date.isolated),decreasing=FALSE)[1:1] #sort by isolation date
...
date.isolated.nz <-ymd(ICMP.NZ$IsolationDateISO, truncated = 3)
sort(table(date.isolated.nz),decreasing=FALSE)[1:1] #sort by isolation date
...
```{r all isolated dates chart, echo=FALSE, warning=FALSE}
...
```{r NZ isolated month chart, echo=FALSE, warning=FALSE}
...
sort(table(ICMP.dump$StandardCollector),decreasing=TRUE)[1:12] #top 12 StandardCollector

sort(table(ICMP.dump$StandardDeterminer),decreasing=TRUE)[1:11] #top 11 StandardDeterminer

sort(table(ICMP.dump$DepositorName),decreasing=TRUE)[1:11] #top 11 DepositorName
...
ICMP.dump$LastUpdatedBy <- sapply(ICMP.dump$UpdatedBy, tolower)
ggplot(ICMP.dump, aes(LastUpdatedBy, fill = SpecimenType)) +
  labs(title = "Last user to update an ICMP record") +
...
This document was compiled on `r date()` using R version `r getRversion()` and Tidyverse version `r packageVersion("tidyverse")`. All figures and code are licensed as [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) attributed to Bevan Weir. Source code is avaliabe at <https://github.com/onco-p53/CIS>.
