...
date: "data as of 14 November 2024"
output:
  html_document:
    code_folding: hide
    df_print: default
...
#import the files
ICMP.as.imported.df <- read_csv("ICMP-export-14-nov-2024.csv",
                                guess_max = Inf,
...
all_species <- ICMP.df |>
  group_by(SpecimenType) |>
  summarise(Total.taxa = n_distinct(CurrentNamePart_C1))

NZ_species <- ICMP.df |>
  filter(StandardCountry_CE1 == "New Zealand") |>
  group_by(SpecimenType) |>
  summarise(NZ.taxa = n_distinct(CurrentNamePart_C1))
...
Only `r round((217/1019)*100, digits = 0)`% of the types are from New Zealand.
...
#make a new column called GeoRef true false off DecimalLatitude_CE1
ICMP.df$GeoRef <- ifelse(!is.na(ICMP.df$DecimalLatitude_CE1), TRUE, FALSE)
...
ggplot() + 
  ...
  theme_void() +
  geom_point(data = ICMP.df, aes(x = DecimalLongitude_CE1, y = DecimalLatitude_CE1), color = "red", size = 1, alpha = 0.2) +
  coord_fixed(1.3)
...
ggplot(ICMP.10county, aes(StandardCountry_CE1, fill=SpecimenType)) +
  ...
  labs(x = "StandardCountry_CE1", y = "number of cultures") +
...
#ICMP.NZ <- subset(ICMP.df,(StandardCountry_CE1 == "New Zealand"))
...
#  theme_void() +
#  geom_point(data = ICMP.no.chat, aes(x = DecimalLongitude_CE1, y = DecimalLatitude_CE1, colour = StandardNewZealandAreaCodes_CE1), size = #1, alpha = 0.2, show.legend = FALSE) +
#  coord_fixed(1.3)
...
ggplot(ICMP.NZ, aes(StandardNewZealandAreaCodes_CE1)) +
...
ICMP.NZ$date.isolated <- ymd(ICMP.NZ$ICMPIsolatedDateISO, truncated = 3)
...
attach(ICMP.NZ) 
month.isolated <- ymd(ICMP.NZ$ICMPIsolatedDateISO, truncated = 1)
...
Top 10 collectors, isolators, identifiers

```{r top collectors etc., echo=FALSE, message=FALSE, warning=FALSE}
...
topDepositor <- ICMP.df |> 
  count(ICMPDepositorStandardName, sort = TRUE) |>
  drop_na() |>
  slice_head(n=10)
...
people.table <- full_join(topCollector, topDeterminer, 
  ...
  full_join(topDepositor,
            by = c("StandardCollector_CE1" = "ICMPDepositorStandardName" ))
