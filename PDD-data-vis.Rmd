...
date: "data as of 8 February 2022"
output:
  html_document:
    code_folding: hide
    df_print: default
...
The PDD has **`r count(PDD.types)` type specimens** specimens. There are different 'kinds' of types. Bacterial types are 'type strains', bacterial pathovars are 'pathotype strains'. Fungal type specimens may have been isolated from (*ex*) a type specimen (e.g. from PDD) and are thus 'ex types'.

```{r types chart, eval=FALSE, include=FALSE}
...
#  theme_void() +
#  geom_point(data = PDD.no.chat, aes(x = DecimalLong, y = DecimalLat, colour = NZAreaCode), size = #1, alpha = 0.2, show.legend = FALSE) +
#  coord_fixed(1.3)
