...
# Ensure LastUpdatedBy is all lowercase
PDD.df$LastUpdatedBy <- sapply(PDD.df$UpdatedBy, tolower)

# Remove rows with "bulkgeorefconversion"
PDD_filtered <- PDD.df |> 
  filter(LastUpdatedBy != "bulkgeorefconversion")

# Create the plot
ggplot(PDD_filtered, aes(LastUpdatedBy, fill = specimen_kind)) +
  geom_bar() +
  labs(
    title = "Last user to update a PDD record",
    x = "User",
